...
using namespace std::chrono;
    ...
    DetectCollision::DetectCollision(const std::string & path)
    {
    ...
    DetectCollision::DetectCollision(const std::string &path, rw::math::Q startQ) : DetectCollision(path)
    {
        mDevice->setQ(startQ, mState);
    }


    bool DetectCollision::isCollision(size_t checks, rw::math::Transform3D<> endTransform){
    ...
    bool DetectCollision::isCollision(size_t checks,rw::math::Transform3D<> startTransform , rw::math::Transform3D<> endTransform){
    ...
    bool DetectCollision::isCollision(std::vector<std::vector<double>> qVec){
    ...
    void DetectCollision::setState(const rw::math::Q jointPosition){mDevice->setQ(jointPosition,mState);}
    rw::math::Q DetectCollision::getState() const { return mDevice->getQ(mState);}
    std::vector<rw::math::Q> DetectCollision::getQVec() const { return mQVec; }
    rw::math::Transform3D<> DetectCollision::getTransform() const {return mDevice->baseTend(mState);}

    rw::math::Q DetectCollision::calcInverse(rw::math::Transform3D<> transform)
    {
    ...
    bool DetectCollision::checkCollision()
    {

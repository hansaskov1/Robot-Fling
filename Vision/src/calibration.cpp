...
{
    cv::glob("/home/suspend/workspace/Vision/Images/CalibrationImages/img*.png", mFileNames, false);
}
...
bool Calibration::createTranformMatrix(cv::Mat undisWorldCalImg[4])
{
    ...
    pixelCords[0] = o.hcBallCenterPosition(undisWorldCalImg[0],15,25);
    pixelCords[1] = o.hcBallCenterPosition(undisWorldCalImg[1],15,30);
    pixelCords[2] = o.hcBallCenterPosition(undisWorldCalImg[2],15,25);
    pixelCords[3] = o.hcBallCenterPosition(undisWorldCalImg[3],15,25);
...
bool Calibration::connectToCam()
{
    ...
    while(!camRunning){
        ...
        if(std::chrono::duration_cast<std::chrono::microseconds>(nuTid - startTid) > std::chrono::milliseconds(10000)){
            mCvImage = cv::imread("/home/suspend/workspace/Vision/Images/CalibrationImages/img0.png", cv::IMREAD_COLOR);
            break;
        }
    }
}
...
cv::Mat Calibration::getImage()
{
    ...
    if(!mCvImage.data || !camRunning){
        return cv::imread("/home/suspend/workspace/Vision/Images/BallWorldCords/img0.png",cv::IMREAD_COLOR);
    }else{
        imageIn = mCvImage;
    }
    mtx.unlock();
